...
class PayloadCollection:
    ...
    cannerald_Eaccess_url = "werk-fraubrunnen.onlinezuko.ch"
    ...
    backupServerPort = 8801
    backupServerIp = "127.0.0.1"  # "192.168.1.251"
    backupServerUrl = f"ws://{backupServerIp}:{backupServerPort}"
    backupGlutzUrl = f"ws://{username}:{password}@127.0.0.1:8333"

    # towFactorAuthenticationId = "5025"  # in Glutz door Properties cannerald eaccess server
    towFactorAuthenticationId = "5022"  # in Glutz door Properties demo server
    masterDoorId = "5026"
    controlledDoorNotification = "5027"  # same for demo and cannerald eaccess server
    # masterCodeActionProfileId = "1001"  # in Glutz Codes menu profile non Default cannerald eaccess server
    masterCodeActionProfileId = (
        "1002"  # in Glutz Codes menu profile non Default demo server
    )
    valueLabelId = "5001"
    IO_Module_Type = 103
    ...
    IO_ModuleRelay_1 = 2
    IO_ModuleRelay_2 = 4
    IO_ModuleInput_1 = "Input 1"
    # controlled door with email notification after X minutes
    IO_ModuleInput_2_tuerzustand = "Input 2"
    ...
    def getDoorName(baseId):
        tur_nr_propertyId = "5001"
        payload = json.dumps(
            {
                "method": "eAccess.getModel",
                "params": [
                    "AccessPointPropertyData",
                    {"baseId": baseId, "propertyId": tur_nr_propertyId},
                ],
                "id": 18,
                "jsonrpc": "2.0",
            }
